...
The CI smoke run stands up local-db kind, loads `seed-small`, then runs `store|query|mixed` capped
...
Currently it can be executed manually, but later will be integrated to CI workflows (TBD).
