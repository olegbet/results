...
type Template struct {
	...
	taskRuns      []childSkeleton
	taskRunWeight float64
}

// childSkeleton is one weighted child TaskRun blueprint. The generator picks
// among a template's skeletons per child to reproduce a realistic size spread.
type childSkeleton struct {
	name   string
	weight float64
	tr     *tektonv1.TaskRun
}

// pickTaskRun selects a child skeleton deterministically by weight. It draws no
// rng value when the template has a single skeleton, keeping single-skeleton and
// default templates byte-identical to the pre-feature behavior.
func (t *Template) pickTaskRun(rng *rand.Rand) *tektonv1.TaskRun {
	if len(t.taskRuns) == 1 {
		return t.taskRuns[0].tr
	}
	target := rng.Float64() * t.taskRunWeight
	var common float64
	for _, s := range t.taskRuns {
		common += s.weight
		if target < common {
			return s.tr
		}
	}
	return t.taskRuns[len(t.taskRuns)-1].tr
}
...
type templateMeta struct {
	...
	ChildSkeletonWeights map[string]float64 `json:"childSkeletonWeights"`
...
func DefaultTemplates() (*TemplateSet, error) {
	return LoadTemplates(embeddedTemplates, "templates/default")
}
...
// optional taskruns/ directory (or a single taskrun.yaml) provides the child
// TaskRun skeletons and an optional template.yaml carries metadata (id, weight,
// child count range, per-skeleton weights).
func LoadTemplates(fsys fs.FS, root string) (*TemplateSet, error) {
	...
	for _, e := range entries {
		...
		fmt.Fprintf(os.Stderr, "Loading template %q from %s...\n", e.Name(), dir)
...
func loadTemplate(fsys fs.FS, dir, name string) (*Template, error) {
	...
	// Read metadata first so per-skeleton weights are available when the child
	// skeleton slice is built.
	meta := templateMeta{}
	...
	skeletons, err := loadChildSkeletons(fsys, dir, meta.ChildSkeletonWeights)
	if err != nil {
		return nil, err
	}
	t.taskRuns = skeletons
	for _, s := range t.taskRuns {
		t.taskRunWeight += s.weight
	}

	return t, nil
}

// loadChildSkeletons resolves a template's child TaskRun blueprints. Precedence:
// a taskruns/ directory of *.yaml skeletons (sorted by filename), else a single
// taskrun.yaml, else a minimal built-in default. Per-skeleton weights default to
// 1.0 and are overridden by matching entries in weights.
func loadChildSkeletons(fsys fs.FS, dir string, weights map[string]float64) ([]childSkeleton, error) {
	trDir := path.Join(dir, "taskruns")
	if entries, err := fs.ReadDir(fsys, trDir); err == nil {
		sort.Slice(entries, func(i, j int) bool { return entries[i].Name() < entries[j].Name() })
		var skeletons []childSkeleton
		for _, e := range entries {
			if e.IsDir() || path.Ext(e.Name()) != ".yaml" {
				continue
			}
			trBytes, err := fs.ReadFile(fsys, path.Join(trDir, e.Name()))
			if err != nil {
				return nil, fmt.Errorf("reading %s/%s: %w", trDir, e.Name(), err)
			}
			tr := &tektonv1.TaskRun{}
			if err := yaml.UnmarshalStrict(trBytes, tr); err != nil {
				return nil, fmt.Errorf("parsing %s/%s: %w", trDir, e.Name(), err)
			}
			...
			skeletons = append(skeletons, childSkeleton{name: name, weight: weight, tr: tr})
		}
		if len(skeletons) > 0 {
	...
	if trBytes, err := fs.ReadFile(fsys, path.Join(dir, "taskrun.yaml")); err == nil {
		...
		return []childSkeleton{{name: "taskrun", weight: 1, tr: tr}}, nil
	}

	return []childSkeleton{{name: "default", weight: 1, tr: defaultTaskRunSkeleton()}}, nil
}
