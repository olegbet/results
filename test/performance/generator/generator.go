...
func New(cfg Config, ts *TemplateSet) (*Generator, error) {
	...
	// Progress logs go to stderr so stdout stays a clean JSON contract for the
	// `dataset` subcommand.
	fmt.Fprintln(os.Stderr, "Creating generator...")
...
func (g *Generator) At(index int) (*Instance, error) {
	...
	inst.TaskRuns = g.buildTaskRuns(tmpl, inst, index, childCount, start, rng)
