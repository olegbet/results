...
func TestContentHash(t *testing.T) {
	cfg := testRunConfig()
	cfg.Count = 20 // keep the materialization cheap
	...
	seedHash, err := contentHash(seed)
	if err != nil {
		t.Fatalf("contentHash(seed): %v", err)
	}
	liveHash, err := contentHash(live)
	if err != nil {
		t.Fatalf("contentHash(live): %v", err)
	}
	if seedHash == "" {
		t.Fatal("seed content hash is empty")
	}
	if liveHash == "" {
		t.Fatal("live content hash is empty")
	}
	if seedHash == liveHash {
		t.Fatal("seed and live content hashes must differ (disjoint UID partitions)")
	}

	seedAgain, err := newGenerator(cfg, false)
	if err != nil {
		t.Fatalf("seed generator (again): %v", err)
	}
	again, err := contentHash(seedAgain)
	if err != nil {
		t.Fatalf("contentHash(seed again): %v", err)
	}
	if again != seedHash {
		t.Errorf("contentHash not deterministic: %s != %s", again, seedHash)
	}
}
