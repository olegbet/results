...
// contentHash returns the deterministic DatasetDefinition content hash for g.
// Every benchmark mode embeds this in meta.dataset_hash so reports can enforce
// like-for-like comparisons across runs.
func contentHash(g *generator.Generator) (string, error) {
	def, err := g.Definition()
	if err != nil {
		return "", fmt.Errorf("computing dataset definition: %w", err)
	}
	return def.ContentHash, nil
}
